...
import { ChevronRight, ChevronLeft, MapPin, Sun, Volume2, Home, Users, Save } from 'lucide-react';
import { regionsTowns } from '../data/regionsTowns';
import { formatProfileForSave, simulateSaveProfile } from '../utils/profileApi';
...
export const PreferenceCenter = ({
  ...
  const [selectedRegion, setSelectedRegion] = useState<string | null>(availableRegions[0] || null);
  const [isSaving, setIsSaving] = useState(false);
  const [saveMessage, setSaveMessage] = useState<string | null>(null);

  const formatPrice = (price: number) => `$${(price / 1000000).toFixed(1)}m`;

  const getTownsForRegion = (region: string): string[] => {
    return regionsTowns[region] || [];
  };
      ...
      case 0:
        return (
          <div className="space-y-8">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Preferred Location</h3>
              <div className="space-y-4">
                <div>
                ...
                {selectedRegion && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-3">
                      Towns in {selectedRegion}
                    </label>
                    <div className="flex gap-2 flex-wrap">
                      {getTownsForRegion(selectedRegion).map(town => (
                        <button
                          key={town}
                          onClick={() => {
                            const newRegions = constraints.regions.includes(town)
                              ? constraints.regions.filter(r => r !== town)
                              : [...constraints.regions, town];
                            onConstraintsChange({ ...constraints, regions: newRegions });
                          }}
                          className={`px-4 py-2 rounded-lg font-medium transition-all ${
                            constraints.regions.includes(town)
                              ? 'bg-teal-600 text-white shadow-lg'
                              : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                          }`}
                        >
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      {constraints.regions.length === 0
                        ? `Select towns to narrow search, or leave empty to include all towns in ${selectedRegion}`
                        : `Selected: ${constraints.regions.length} town(s)`}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Minimum Lease Remaining</h3>
              <div className="space-y-4">
                <div className="text-center">
                  <div className="text-4xl font-bold text-teal-600 mb-2">
                    {constraints.minLeaseRemaining} years
                  </div>
                  <p className="text-sm text-gray-500">
                    We'll only show properties with at least this much lease time left
                  </p>
                </div>
                <div className="space-y-2">
                  <input
                    ...
                    max="99"
                    step="1"
                    value={constraints.minLeaseRemaining}
                    onChange={(e) => onConstraintsChange({
                      ...constraints,
                      minLeaseRemaining: Number(e.target.value)
                    })}
                    className="w-full h-3 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-teal-600"
                  />
                  <div className="flex justify-between text-xs text-gray-500">
              ...
              <div key={factor.name} className="border border-gray-200 rounded-lg p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    {factor.name === 'Solar Orientation' && <Sun className="w-5 h-5 text-teal-600" />}
                    {factor.name === 'Acoustic Comfort' && <Volume2 className="w-5 h-5 text-teal-600" />}
                    {factor.name === 'Convenience' && <MapPin className="w-5 h-5 text-teal-600" />}
                    ...
                    {factor.name === 'Convenience' && (
                      <div className="space-y-4">
                        <label className="block text-sm font-medium text-gray-700">
                          Select Amenities You Care About
                        </label>
                        <div className="grid grid-cols-2 gap-3">
                          {['Parents Address', 'Schools', 'Hawker Centres', 'Supermarkets', 'Parks', 'Hospitals'].map(amenity => (
                            <button
                              key={amenity}
                              onClick={() => {
                                const selected = factor.selectedAmenities || [];
                                const newSelected = selected.includes(amenity)
                                  ? selected.filter(a => a !== amenity)
                                  : [...selected, amenity];
                                updateFactor(index, { selectedAmenities: newSelected });
                              }}
                              className={`py-2 px-3 rounded-lg font-medium transition-all text-sm ${
                                (factor.selectedAmenities || []).includes(amenity)
                                  ? 'bg-teal-600 text-white shadow-lg'
                                  : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                              }`}
                            >
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg text-base focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                />
              </div>
      ...
      case 3:
        return (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-teal-50 to-teal-100 rounded-lg p-6 space-y-4">
              <div>
                ...
                <ul className="text-sm text-gray-700 space-y-1 ml-6">
                  ...
                  <li>Minimum Lease: {constraints.minLeaseRemaining} years</li>
                </ul>
              </div>
                ...
                <ul className="text-sm text-gray-700 space-y-1 ml-6">
                  {factors.map(f => (
                    <li key={f.name}>
                      ...
                      {f.selectedAmenities && f.selectedAmenities.length > 0 && (
                        <span className="text-xs text-gray-600"> - {f.selectedAmenities.join(', ')}</span>
                      )}
                    </li>
                  ))}
                </ul>
              ...
              <div>
                ...
                <ul className="text-sm text-gray-700 space-y-1 ml-6">
                  <li>A: {commuterConfig.addressA || 'Not set'}</li>
                  <li>B: {commuterConfig.addressB || 'Not set'}</li>
              ...
              <div className="flex-1 h-1 mx-2 rounded-full bg-gray-200">
                <div
                  className={`h-full rounded-full transition-all ${
                    index < currentStep ? 'w-full bg-teal-500' : 'w-0'
                  }`}
                ></div>
              </div>
            </div>
        ...
        <div className="flex justify-between items-center gap-4">
          ...
          {currentStep === steps.length - 1 ? (
            <div className="flex gap-3">
              {isLoggedIn && (
                <button
                  onClick={async () => {
                    setIsSaving(true);
                    try {
                      const payload = formatProfileForSave(userId, constraints, factors, commuterConfig);
                      await simulateSaveProfile(payload);
                  ...
                  disabled={isSaving}
                  className="flex items-center gap-2 px-6 py-3 bg-gray-600 text-white rounded-lg font-medium hover:bg-gray-700 transition-all disabled:opacity-50"
                >
              ...
              <button
                onClick={onComplete}
                className="flex items-center gap-2 px-6 py-3 bg-teal-600 text-white rounded-lg font-medium hover:bg-teal-700 transition-all shadow-lg hover:shadow-xl"
              >
                Analyze Properties
